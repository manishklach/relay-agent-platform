...
export type Actor = { id: string; email: string; role: 'owner' | 'builder' | 'operator' | 'viewer' };

export async function requireActor(request: NextRequest, minimumRole: Actor['role'] = 'viewer'): Promise<Actor> {
  await ensureDatabase();
  const user = await getChatGPTUser();
  const hostname = new URL(request.url).hostname;
  const isLocal = hostname === 'localhost' || hostname === '127.0.0.1';
  if (!user && !isLocal) throw new Response('Authentication required.', { status: 401 });

  const id = user?.userId ?? 'local-dev';
  const email = user?.email ?? 'local@relay.dev';
  const existing = await env.DB.prepare(
    'SELECT role FROM workspace_members WHERE workspace_id = ? AND user_id = ?',
  ).bind(DEFAULT_WORKSPACE_ID, id).first<{ role: Actor['role'] }>();

  let role = existing?.role;
  if (!role) {
    const count = await env.DB.prepare(
      'SELECT COUNT(*) AS count FROM workspace_members WHERE workspace_id = ?',
    ).bind(DEFAULT_WORKSPACE_ID).first<{ count: number }>();
    ...
    ).bind(DEFAULT_WORKSPACE_ID, id, email, role, Date.now()).run();
  }

  if (roleRank(role) < roleRank(minimumRole)) {
    throw new Response(`Role ${minimumRole} or higher is required.`, { status: 403 });
  }
...
export async function getAgent(agentId: string): Promise<AgentConfig | null> {
  ...
  ).bind(agentId, DEFAULT_WORKSPACE_ID).first<Record<string, unknown>>();
...
export async function writeAudit(actorId: string, action: string, targetType: string, targetId: string, metadata: Record<string, unknown> = {}) {
  await env.DB.prepare(
    `INSERT INTO audit_logs (id, workspace_id, actor_id, action, target_type, target_id, metadata_json, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
  ).bind(crypto.randomUUID(), DEFAULT_WORKSPACE_ID, actorId, action, targetType, targetId, JSON.stringify(metadata), Date.now()).run();
}
