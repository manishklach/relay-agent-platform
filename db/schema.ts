...
export const workspaceMembers = sqliteTable(
  'workspace_members',
  {
    ...
    role: text('role', { enum: ['owner', 'builder', 'operator', 'viewer'] }).notNull(),
    createdAt: integer('created_at').notNull(),
  },
  (table) => [
...
export const runs = sqliteTable(
  'runs',
  {
    ...
    status: text('status', { enum: ['running', 'succeeded', 'failed', 'waiting_approval'] }).notNull(),
...
export const runSteps = sqliteTable(
  'run_steps',
  {
    ...
    kind: text('kind', { enum: ['model', 'tool', 'guardrail', 'approval'] }).notNull(),
    name: text('name').notNull(),
    status: text('status', { enum: ['succeeded', 'failed', 'blocked', 'pending'] }).notNull(),
    inputJson: text('input_json').notNull().default('{}'),
    outputJson: text('output_json').notNull().default('{}'),
    durationMs: integer('duration_ms').notNull().default(0),
    createdAt: integer('created_at').notNull(),
  },
  (table) => [index('idx_run_steps_run_sequence').on(table.runId, table.sequence)],
);
...
export const approvals = sqliteTable(
  ...
  (table) => [index('idx_approvals_workspace_status').on(table.workspaceId, table.status)],
);
...
export const evaluationCases = sqliteTable(
  ...
  {
    id: text('id').primaryKey(),
    suiteId: text('suite_id').notNull().references(() => evaluationSuites.id),
    name: text('name').notNull(),
    input: text('input').notNull(),
    expectedJson: text('expected_json').notNull(),
    graderType: text('grader_type', { enum: ['contains', 'not_contains', 'json'] }).notNull(),
    createdAt: integer('created_at').notNull(),
  },
...
export const evaluationRuns = sqliteTable(
  ...
  (table) => [index('idx_eval_runs_suite_created').on(table.suiteId, table.createdAt)],
);
